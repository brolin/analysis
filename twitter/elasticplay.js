...
(function match() {
  ...
  es.search('twitter', 'tweet', qryObj).on('data', function(data) {
...
(function createTwitterIndex() {
  ...
  var settings = {
    ...
    function createIndex() {
      es.createIndex('twitter', settings).on('data', function(data) {
        console.log(data);
      }).exec();
    }
});
...
(function createAnalysisIndex() {
        ...
        "analysis": {
          ...
          "analyzer": {
            "es_tweetAnalyzer": {
              ...
              "filter": ["icu_folding", "icu_normalizer", "es_stop_filter"]
            },
            "shinglesAnalyzer": {
              "type": "custom",
              "tokenizer": "standard",
              "char_filter": ["remove_punctuation"],
              "filter": ["icu_folding", "icu_normalizer", "shingles_filter"]
            }
          }
        }
      }
  ...
  es.deleteIndex('stream', createIndex);

  function createIndex() {
    es.createIndex('stream', settings).on('data', function(data) {
      console.log(data);
    }).exec();
  }
});
...
(function createNyNIndex() {
  ...
  function createIndex() {
    es.createIndex('nocheyniebla', settings).on('data', function(data) {
      console.log(data);
    }).exec();
  }
});
...
(function exportReporteToCaso() {
  ...
  es.search('nocheyniebla', 'reporte', {}, {
    search_type: 'scan',
    scroll: '60m',
    size: 300
  }, function(err, data) {
...
(function updateSettings() {
  ...
  es.closeIndex('twitter').on('data', function(data) {
    console.log(data);
    updateSettings();
  });

  function updateSettings(done) {
    es.updateSettings('twitter', settings).on('data', function(data) {
  ...
  function save(data, cb) {
    ...
    data.forEach(function(d) {
      var item = {
        ...
        text: d._source.text,
        term: d._source.term,
        urls: d._source.entities.urls.map(function(u) {
